...
**Status (2026-06):** calibration (Brier/log + reliability), providers/custom, steering, and the SSE-traced internet-lookup tiers have landed. DSPy optimization has its first scaffold — `optimize/scorer_opt.py` (`build_trainset()` → `dspy.Example`s from `forecast_resolutions`, a title-aligned `brier_metric`, and `optimize_scorer()` selecting BootstrapFewShot/MIPROv2 by data volume, saving a compiled program). It is **offline / opt-in and currently a NO-OP**: too few forecasts have resolved (< `MIN_EXAMPLES`=8) and it is not yet wired into startup load nor a CI eval split. The `prompts` API and prompt-loader write-back loop remain to be ported. Cutover readiness is assessed in [`docs/CUTOVER.md`](./CUTOVER.md).
